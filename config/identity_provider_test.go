...
// provider names in the config file become lowercase keys. Route references
// (Route.identity_providers) are list values and are NOT lowercased, which is
// why validateProviderName rejects mixed-case names outright.
...
func TestValidateIdentityProviders(t *testing.T) {
	...
	t.Run("mixed-case provider name rejected", func(t *testing.T) {
		// Viper lowercases config-file map keys but not route references, so a
		// mixed-case name could never be referenced from a route. Names sourced
		// from the databroker (ApplySettings) skip viper, so validation is the
		// only gate.
		o := NewDefaultOptions()
		o.IdentityProviders = map[string]IdentityProvider{"K8s-Prod": provider}
	...
	t.Run("duplicate issuer error is deterministic", func(t *testing.T) {
		...
		// Sorted-name iteration: "a" is always seen first.
		for range 5 {
			err := o.validateIdentityProviders()
			assert.ErrorContains(t, err, `used by both "a" and "b"`)
		}
	})
