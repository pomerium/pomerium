...
func getUpstreamProtocolForPolicy(_ context.Context, options *config.Options, policy *config.Policy) upstreamProtocolConfig {
	upstreamProtocol := upstreamProtocolAuto
	if policy.GetAllowWebsockets(options) {
