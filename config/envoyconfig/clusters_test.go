...
func Test_BuildClusters(t *testing.T) {
	...
	clusters, err := b.NewForConfig(&config.Config{Options: opts}).BuildClusters(ctx)
...
func Test_buildPolicyTransportSocket(t *testing.T) {
	...
	rootCA := b.FileManager.BytesDataSource("ca.pem", rootCABytes).GetFilename()
	...
	combinedCA := b.FileManager.BytesDataSource("ca.pem", combinedCABytes).GetFilename()

	t.Run("insecure", func(t *testing.T) {
		ts, err := b.NewForConfig(&config.Config{Options: o1}).buildPolicyTransportSocket(ctx, &config.Policy{
	...
	t.Run("options custom ca", func(t *testing.T) {
		ts, err := b.NewForConfig(&config.Config{Options: o2}).buildPolicyTransportSocket(ctx, &config.Policy{
			To: mustParseWeightedURLs(t, "https://example.com"),
		}, *mustParseURL(t, "https://example.com"))
		require.NoError(t, err)
		testutil.AssertProtoJSONEqual(t, `
...
func Test_buildCluster(t *testing.T) {
	...
	o1 := config.NewDefaultOptions()
	t.Run("insecure", func(t *testing.T) {
		sb := b.NewForConfig(&config.Config{Options: o1})
...
func Test_bindConfig(t *testing.T) {
	...
	t.Run("no bind config", func(t *testing.T) {
		cluster, err := b.NewForConfig(&config.Config{Options: &config.Options{}}).buildPolicyCluster(ctx, &config.Policy{
	...
	t.Run("freebind", func(t *testing.T) {
		cluster, err := b.NewForConfig(&config.Config{Options: &config.Options{
