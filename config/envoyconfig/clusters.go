...
func (b *Builder) BuildClusters(ctx context.Context, cfg *config.Config) ([]*envoy_config_cluster_v3.Cluster, error) {
	...
	controlGRPC, err := b.buildInternalCluster(ctx, cfg, "pomerium-control-plane-grpc", grpcURLs, upstreamProtocolHTTP2, Keepalive(true))
	if err != nil {
		return nil, err
	}
	...
	controlDebug, err := b.buildInternalCluster(ctx, cfg, "pomerium-control-plane-debug", []*url.URL{debugURL}, upstreamProtocolAuto, Keepalive(false))
	if err != nil {
		return nil, err
	}
