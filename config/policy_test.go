...
func TestPolicy_GetAllowSPDY(t *testing.T) {
	...
	assert.True(t, (&Policy{AllowUpgrades: nullable.From([]string{"spdy/3.1"})}).GetAllowSPDY(nil))
	assert.True(t, (&Policy{AllowUpgrades: nullable.From([]string{"SpDy/3.1"})}).GetAllowSPDY(nil))
	assert.True(t, (&Policy{}).GetAllowSPDY(&Options{AllowUpgrades: nullable.From([]string{"spdy/3.1"})}))
	assert.True(t, (&Policy{}).GetAllowSPDY(&Options{AllowUpgrades: nullable.From([]string{"SpDy/3.1"})}))
}
...
func TestPolicy_GetAllowUpgrades(t *testing.T) {
	...
	assert.Equal(t, []string{"a", "b", "c"},
		(&Policy{AllowUpgrades: nullable.NewValue(true, []string{"a", "b", "c"})}).GetAllowUpgrades(nil))
	assert.Equal(t, []string{"a", "b", "c"},
		(&Policy{AllowUpgrades: nullable.From([]string{"a", "b", "c"})}).GetAllowUpgrades(&Options{
...
func TestPolicy_GetAllowWebsockets(t *testing.T) {
	...
	assert.True(t, (&Policy{AllowUpgrades: nullable.From([]string{"websocket"})}).GetAllowWebsockets(nil))
	assert.True(t, (&Policy{AllowUpgrades: nullable.From([]string{"WeBsOcKeT"})}).GetAllowWebsockets(nil))
	assert.True(t, (&Policy{}).GetAllowWebsockets(&Options{AllowUpgrades: nullable.From([]string{"websocket"})}))
	assert.True(t, (&Policy{}).GetAllowWebsockets(&Options{AllowUpgrades: nullable.From([]string{"WeBsOcKeT"})}))
}
