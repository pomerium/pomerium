...
func (o *Options) GetSetResponseHeadersForPolicy(policy *Policy) map[string]string {
	...
	if o.SetResponseHeaders == nil {
		...
		if !o.HasCertificates() || o.UseStaging {
...
func (o *Options) ApplySettings(ctx context.Context, certsIndex *cryptutil.CertificatesIndex, settings *config.Settings) {
	...
	set(&o.Enable, settings.Autocert)
	set(&o.AutocertOptions.CA, settings.AutocertCa)
	set(&o.Email, settings.AutocertEmail)
	set(&o.EABKeyID, settings.AutocertEabKeyId)
	set(&o.EABMACKey, settings.AutocertEabMacKey)
	set(&o.UseStaging, settings.AutocertUseStaging)
	set(&o.MustStaple, settings.AutocertMustStaple)
	set(&o.Folder, settings.AutocertDir)
	set(&o.TrustedCA, settings.AutocertTrustedCa)
...
func (o *Options) ToProto() *config.Config {
	...
	copySrcToOptionalDest(&settings.Autocert, &o.Enable)
	copySrcToOptionalDest(&settings.AutocertCa, &o.AutocertOptions.CA)
	copySrcToOptionalDest(&settings.AutocertEmail, &o.Email)
	copySrcToOptionalDest(&settings.AutocertEabKeyId, &o.EABKeyID)
	copySrcToOptionalDest(&settings.AutocertEabMacKey, &o.EABMACKey)
	copySrcToOptionalDest(&settings.AutocertDir, &o.Folder)
	copySrcToOptionalDest(&settings.AutocertTrustedCa, &o.TrustedCA)
	copySrcToOptionalDest(&settings.AutocertUseStaging, &o.UseStaging)
	copySrcToOptionalDest(&settings.AutocertMustStaple, &o.MustStaple)
