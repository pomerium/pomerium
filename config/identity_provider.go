...
func (p IdentityProvider) Validate() error {
	...
	if len(p.Audiences) == 0 {
		return fmt.Errorf("identity_providers[%s]: at least one audience is required", p.Issuer)
	}
	for _, alg := range p.SupportedAlgs {
		if _, ok := validSigningAlgs[alg]; !ok {
			return fmt.Errorf("identity_providers[%s]: unsupported signing algorithm %q "+
				"(allowed: RS/PS/ES 256/384/512, EdDSA)", p.Issuer, alg)
		}
	}
	return nil
}
...
func (o *Options) validateIdentityProviders() error {
	...
	for name, ip := range o.IdentityProviders {
