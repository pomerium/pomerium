...
`pomerium-cli` has been factored out of the core repository and now resides at https://github.com/pomerium/cli. If you currently install from [Packages](docs/releases.html#packages) or [Homebrew](/docs/releases.html#homebrew), no changes should be required to your process. However, users of docker images or direct github release downloads will need to update their references.
