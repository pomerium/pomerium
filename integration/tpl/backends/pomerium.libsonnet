...
local Environment(mode, idp, authentication_flow, dns_suffix) =
  {
    ...
    ACCESS_LOG_FIELDS: std.join(',', [
      ...
      "tls-local-cert",
      "tls-peer-cert",
      "x-downstream-wire-bytes-sent=.common_properties.downstream_wire_bytes_sent",
      "x-downstream-wire-bytes-received=.common_properties.downstream_wire_bytes_received",
      "x-upstream-wire-bytes-sent=.common_properties.upstream_wire_bytes_sent",
      "x-upstream-wire-bytes-received=.common_properties.upstream_wire_bytes_received",
    ]),
    IDP_PROVIDER: idp,
