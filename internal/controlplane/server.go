...
type Server struct {
	...
	// loopback port. The AS is reached only through ordinary routes pointing here
	// (`to: pomerium://agentic`), and those routes are what authorize it, so
	// nothing else may be able to reach this listener.
...
func NewServer(
	...
	// The agentic authorization server's listener. Like the other control-plane
	// listeners it binds an allocated loopback port; routes reach it as
	// `to: pomerium://agentic`, which the Envoy config resolves to this port.
	srv.AgenticListener, err = reuseport.Listen("tcp4", net.JoinHostPort("127.0.0.1", cfg.AgenticPort))
	if err != nil {
		return nil, fmt.Errorf("error binding the agentic authorization server listener: %w", err)
	}
