...
func TestSessionHandleReader(t *testing.T) {
	...
	h1 := &session.Handle{
		...
		DataBrokerServerVersion: proto.Uint64(1234),
		DataBrokerRecordVersion: proto.Uint64(5678),
	}
...
func TestSessionHandleWriter(t *testing.T) {
	...
	h := &session.Handle{
		Id:                      "ID",
		Audience:                []string{"AUDIENCE1", "AUDIENCE2"},
		IdentityProviderId:      proto.String("IDP_ID"),
		UserId:                  proto.String("USER_ID"),
		DataBrokerServerVersion: proto.Uint64(1234),
		DataBrokerRecordVersion: proto.Uint64(5678),
	}
	...
	shw := sessions.NewSessionHandleWriter(k1, "_session", "example.com", true, true, time.Hour, http.SameSiteStrictMode)
