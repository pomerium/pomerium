...
func TestNewQueryParamStore(t *testing.T) {
	tests := []struct {
		...
		{"simple good", &session.Handle{}, mock.Encoder{MarshalResponse: []byte("ok")}, "", false, &url.URL{Path: "/", RawQuery: "pomerium_session=ok"}},
		{"marshall error", &session.Handle{}, mock.Encoder{MarshalError: errors.New("error")}, "", true, &url.URL{Path: "/"}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := got.SaveSession(w, r, tt.handle); (err != nil) != tt.wantErr {
