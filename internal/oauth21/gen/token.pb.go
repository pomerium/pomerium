...
type TokenRequest struct {
	...
	// REQUIRED when authenticating with private_key_jwt or client_secret_jwt.
	// The signed JWT bearing the client's authentication assertion.
	ClientAssertion *string `protobuf:"bytes,8,opt,name=client_assertion,json=clientAssertion,proto3,oneof" json:"client_assertion,omitempty"`
	// REQUIRED when client_assertion is present. RFC 7523 Section 2.2 defines the only
	// value in use today, but the accepted set is deliberately not constrained here:
	// this message is persisted, and pinning the value in the storage layer would
	// reject records written by a future version that accepts another assertion type.
	// The handler enforces the value it supports at authentication time.
	ClientAssertionType *string `protobuf:"bytes,9,opt,name=client_assertion_type,json=clientAssertionType,proto3,oneof" json:"client_assertion_type,omitempty"`
...
const file_token_proto_rawDesc = "" +
	"\n" +
	"\vtoken.proto\x12\aoauth21\x1a\x1bbuf/validate/validate.proto\x1a\x1fgoogle/protobuf/timestamp.proto\"\x8f\t\n" +
	"\fTokenRequest\x12[\n" +
	"\n" +
	...
	"\rclient_secret\x18\a \x01(\tB\a\xbaH\x04r\x02\x10\x01H\x05R\fclientSecret\x88\x01\x01\x127\n" +
	"\x10client_assertion\x18\b \x01(\tB\a\xbaH\x04r\x02\x10\x01H\x06R\x0fclientAssertion\x88\x01\x01\x127\n" +
	"\x15client_assertion_type\x18\t \x01(\tH\aR\x13clientAssertionType\x88\x01\x01:\x9c\x04\xbaH\x98\x04\x1a\xa8\x01\n" +
	...
	"<token_request.refresh_token_required_for_refresh_token_grant\x12<refresh_token is required when grant_type is 'refresh_token'\x1a?(this.grant_type != 'refresh_token') || has(this.refresh_token)\x1a\xaa\x01\n" +
	"#token_request.client_assertion_pair\x12Dclient_assertion and client_assertion_type must be provided together\x1a=has(this.client_assertion) == has(this.client_assertion_type)B\a\n" +
	"\x05_codeB\x10\n" +
	...
	"\x06_scopeB\x10\n" +
	"\x0e_client_secretB\x13\n" +
	"\x11_client_assertionB\x18\n" +
	"\x16_client_assertion_type\"\xe1\x02\n" +
	"\rTokenResponse\x12*\n" +
