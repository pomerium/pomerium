...
// the AgenticRun secondary index registered — the seal lookup is a query against
...
func TestQueryRunByBoundClaimsIndexFindsApprovedBeyondAPage(t *testing.T) {
	...
	approved := &oauth21proto.AgenticRun{
		Id:               "run-zz-approved",
		BoundClaimsIndex: idx,
	...
	assert.Equal(t, oauth21proto.AgenticRunState_AGENTIC_RUN_STATE_APPROVED, got.GetState())
}
