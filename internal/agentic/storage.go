...
func GetRunRecordVersion(ctx context.Context, client databroker.DataBrokerServiceClient, id string) (*oauth21proto.AgenticRun, uint64, error) {
	res, err := client.Get(ctx, &databroker.GetRequest{
		Type: protoutil.GetTypeURL(new(oauth21proto.AgenticRun)),
		Id:   id,
	})
...
func PutRunIfUnchanged(ctx context.Context, client databroker.DataBrokerServiceClient, run *oauth21proto.AgenticRun, version uint64) error {
...
func buildRunSession(idpSess *idpsessionpb.IDPSession, run *oauth21proto.AgenticRun, now time.Time) *session.Session {
...
func approverSubject(run *oauth21proto.AgenticRun) string {
	if !IsApproved(run) {
		return ""
	}
	return run.GetSub()
...
func stateString(run *oauth21proto.AgenticRun) string {
	if IsApproved(run) {
		return "approved"
	}
