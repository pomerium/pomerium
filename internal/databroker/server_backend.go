...
func (srv *backendServer) setupRequiredIndex(ctx context.Context, backend storage.Backend) error {
	...
	// AgenticRun is resolved at token bind from the executor's attested claims:
	...
	// creation.
	if err := backend.SetOptions(ctx, "type.googleapis.com/oauth21.AgenticRun", &databrokerpb.Options{
		IndexableFields: []string{
			"bound_claims_index",
		},
