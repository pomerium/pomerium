...
// port of pkg/identity/manager/schedulers_test.go
...
func TestRefreshIDPSessionScheduler_CoolOff(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		defer rss.Stop()
		rss.Update(&idpsession.IDPSession{
			OauthToken: &idpsession.OAuthToken{ExpiresAt: timestamppb.Now()},
		})
...
func TestRefreshIDPSessionScheduler_AccessTokenExpiration(t *testing.T) {
	t.Parallel()
	...
	synctest.Test(t, func(t *testing.T) {
		...
		var rss *refreshIDPSessionScheduler
...
func TestRefreshIDPSessionScheduler_IDTokenExpiresBeforeAccessToken(t *testing.T) {
	t.Parallel()
	...
	synctest.Test(t, func(t *testing.T) {
		...
		sess := &idpsession.IDPSession{}
		updateTokens := func() {
			sess.OauthToken = &idpsession.OAuthToken{
				ExpiresAt: timestamppb.New(time.Now().Add(75 * time.Minute)),
			}
			sess.IdToken = &idpsession.IDToken{
...
func TestRefreshIDPSessionScheduler_IDTokenNotRefreshed(t *testing.T) {
	t.Parallel()
