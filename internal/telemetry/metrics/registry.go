	...
	r.Do(
		func() {
			...
			r.configChecksum, err = r.registry.AddFloat64Gauge(metrics.ConfigChecksumDecimal,
				...
				metric.WithLabelKeys(metrics.InstallationIDLabel, metrics.ServiceLabel, metrics.ConfigLabel),
			)
			if err != nil {
			...
			r.policyCount, err = r.registry.AddInt64DerivedGauge(metrics.PolicyCountTotal,
				...
				metric.WithLabelKeys(metrics.InstallationIDLabel, metrics.ServiceLabel),
			)
			if err != nil {
...
// have this exported
func (r *metricRegistry) setBuildInfo(installationID, service, hostname string) {
	if registry.buildInfo == nil {
		return
	}
	m, err := registry.buildInfo.GetEntry(
		metricdata.NewLabelValue(installationID),
...
func (r *metricRegistry) addPolicyCountCallback(installationID, service string, f func() int64) {
	if r.policyCount == nil {
		return
	}
	err := r.policyCount.UpsertEntry(f, metricdata.NewLabelValue(installationID), metricdata.NewLabelValue(service))
	if err != nil {
		log.Error().Err(err).Msg("telemetry/metrics: failed to get policy count metric")
	}
}

func (r *metricRegistry) setConfigChecksum(installationID, service string, configName string, checksum uint64) {
	if r.configChecksum == nil {
		return
	}
	m, err := r.configChecksum.GetEntry(metricdata.NewLabelValue(installationID), metricdata.NewLabelValue(service), metricdata.NewLabelValue(configName))
	if err != nil {
...
func (r *metricRegistry) addInt64DerivedGaugeMetric(name, desc, installationID, service string, f func() int64) {
	...
	err = m.UpsertEntry(f, metricdata.NewLabelValue(installationID), metricdata.NewLabelValue(service))
	if err != nil {
...
func (r *metricRegistry) addInt64DerivedCumulativeMetric(name, desc, installationID, service string, f func() int64) {
