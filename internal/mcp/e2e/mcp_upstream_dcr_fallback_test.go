...
func TestMCPUpstreamOAuthDCRFallback(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "PRM origin fallback disabled",
			allowFallback: false,
			expectSuccess: false,
		},
		{
			name:          "PRM origin fallback enabled",
			allowFallback: true,
			expectSuccess: true,
		},
	}
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			env.Add(testenv.ModifierFunc(func(_ context.Context, cfg *config.Config) {
				...
				cfg.Options.RuntimeFlags[config.RuntimeFlagMCPAllowPRMSameDomainOrigin] = tc.allowFallback
	...
	// Bootstrap upstream OAuth session via ConnectGet before MCP initialize.
		...
		"?redirect_url=" + url.QueryEscape(redirectTarget)
			...
			if !tc.expectSuccess {
				require.Error(t, err)
				assert.True(t,
					strings.Contains(err.Error(), "Bad Gateway") || strings.Contains(err.Error(), "Unauthorized"),
					"expected gateway auth failure when PRM origin fallback is disabled")
				return
			}
			require.NoError(t, err)
			defer session.Close()
