...
func TestMCPClientRoutes(t *testing.T) {
	...
	t.Run("connect redirect_url to non-client host returns 400", func(t *testing.T) {
		client := authenticatedClient(t, server1Route)
		// Use a completely different host — it's neither an MCP client nor the server itself.
		connectURL := server1Route.URL().Value() + endpoints.PathPomeriumMCPConnect +
			"?redirect_url=" + url.QueryEscape("https://unknown-third-party.example.com/callback")
		req, err := http.NewRequestWithContext(ctx, http.MethodGet, connectURL, nil)
		require.NoError(t, err)
	...
	t.Run("connect redirect_url to same server host succeeds", func(t *testing.T) {
		client := authenticatedClient(t, server1Route)
		// The routes portal lives on the same host as the MCP server —
		// this should be allowed for the portal connect flow.
		connectURL := server1Route.URL().Value() + endpoints.PathPomeriumMCPConnect +
			"?redirect_url=" + url.QueryEscape(server1Route.URL().Value()+"/.pomerium/routes")
		req, err := http.NewRequestWithContext(ctx, http.MethodGet, connectURL, nil)
		require.NoError(t, err)
