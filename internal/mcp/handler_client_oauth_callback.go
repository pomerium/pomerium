...
func (srv *Handler) ClientOAuthCallback(w http.ResponseWriter, r *http.Request) {
	...
	// No linked authorization request — this callback was triggered by the ext_proc
	// 401-intercept flow (not the MCP connect flow). Redirect back to the upstream
	// URL the original request was targeting so the client can retry with the
	// newly-acquired token. OriginalUrl is always set server-side from route config
	// or the ext_proc request path; it is never client-supplied.
