...
const SignInVerifyPage: FC<SignInVerifyPageProps> = ({ data }) => {
  ...
  const expiresDate = parseToDate((data as any).expiresAt ?? data.expiresAt);
   // mock metadata for now, prefer values from `data` if present
  // const issuedAt = (data as any).issuedAt ?? new Date().toLocaleString();
  // const sourceAddress = (data as any).sourceAddress ?? "127.0.0.1";
  // const expiresAt =
  ...
  return (
    <>
      ...
      <Box sx={{ maxWidth: 540, mx: "auto", px: 2 }}>
        {/* Remember me placed above the action buttons and tied to the authorize form */}
