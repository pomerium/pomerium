...
type recordAtomic struct {
	*record
	version uint64
}

func (r *recordAtomic) compare(other *recordAtomic) (shouldReplace bool) {
	// always consider a status with a newer version to replace existing one
	if r.version < other.version {
		return true
	}

	// starting should always replaced by other statuses
	if r.status != StatusStarting && other.status == StatusStarting {
		return false
	}

	return r.Equals(other.record)
}
...
// func (d *DeduplicatorBroker) newOKRecord(
// 	attrs []Attr,
// ) *recordAtomic {
// 	return d.newRecord(StatusRunning, nil, attrs)
// }

// func (d *DeduplicatorBroker) newErrorRecord(
// 	err error,
// 	attrs []Attr,
// ) *recordAtomic {
// 	return d.newRecord(StatusError, err, attrs)
// }

// func (d *DeduplicatorBroker) newRecord(status Status, err error, attrs []Attr) *recordAtomic {
// 	return &recordAtomic{
// 		version: d.innerVersion.Load(),
// 		record:  newRecord(status, err, attrs),
// 	}
// }

func (d *DeduplicatorBroker) Reset() {
	d.innerVersion.Add(1)
}
