...
func (cli *SshCli) AddLogoutCommand(ctrl ChannelControlInterface) {
...
func (cli *SshCli) AddWhoamiCommand(ctrl ChannelControlInterface) {
	cli.AddCommand(&cobra.Command{
		Use: "whoami",
		RunE: func(cmd *cobra.Command, args []string) error {
...
var Handoff = errors.New("handoff")

func (cli *SshCli) AddPortalCommand(ctrl ChannelControlInterface) {
	cli.AddCommand(&cobra.Command{
		Use:   "portal",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if username != cli.username {
				return status.Errorf(codes.Internal, "internal error: username mismatch")
			}
			if hostname == "" {
				return status.Errorf(codes.Internal, "internal error: hostname is empty")
			}
