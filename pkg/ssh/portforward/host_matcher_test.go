...
func TestGlobMatcher(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run("", func(t *testing.T) {
			matcher := GlobHostMatcher(tt.inputPattern)
...
func TestGlobMatcher_EquivalentPatterns(t *testing.T) {
	...
	for _, lhs := range []string{a, b, c, d, e} {
		for _, rhs := range []string{a, b, c, d, e} {
			assert.True(t, GlobHostMatcher(lhs).Equivalent(rhs))
		}
	}

	assert.True(t, GlobHostMatcher("foo*").Equivalent("foo*"))
	assert.True(t, GlobHostMatcher("foo*").Equivalent("foo**"))
	assert.False(t, GlobHostMatcher("foo*").Equivalent("*foo*"))
	assert.True(t, GlobHostMatcher("*foo*").Equivalent("**foo*"))
	assert.True(t, GlobHostMatcher("*foo*").Equivalent("**foo**"))
	assert.True(t, GlobHostMatcher("foo*bar").Equivalent("foo**bar"))
	assert.True(t, GlobHostMatcher("foo**bar").Equivalent("foo***bar"))
}

func TestStringMatcher(t *testing.T) {
	{
		matcher := StringHostMatcher("test")
