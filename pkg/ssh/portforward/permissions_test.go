...
func TestPermissionSet_CancelReset(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run("CancelReset", func(t *testing.T) {
			...
			tc.perm1.Context, ca1 = context.WithCancelCause(context.Background())
			ps.Add(tc.perm1)

			var ca2 context.CancelCauseFunc
			tc.perm2.Context, ca2 = context.WithCancelCause(context.Background())
			ps.Add(tc.perm2)
			...
			ps.ResetCanceled(uint(port1), context.Background())
			...
			ps.ResetCanceled(uint(port2), context.Background())
			{
				p, ok := ps.Find(host1, port1)
				assert.True(t, ok)
				assert.Same(t, tc.perm1, p)
...
func TestPermissionSet_CancelResetDynamic(t *testing.T) {
	...
	ps.ResetCanceled(1234, context.Background())
	...
	assert.Panics(t, func() {
		ps.ResetCanceled(0, context.Background())
	})
}
...
func TestPermissionSet_CancelResetThenRemove(t *testing.T) {
	...
	ctx2, ca2 := context.WithCancelCause(context.Background())
	defer ca2(errors.New("unused"))

	ps.ResetCanceled(1234, ctx2)
...
func TestPermissionSet_Match(t *testing.T) {
	...
	canceled1 := &portforward.Permission{
		Context: func() context.Context {
			canceled, ca := context.WithCancel(context.Background())
			ca()
			return canceled
		}(),
		HostMatcher:   portforward.GlobMatcher("three"),
