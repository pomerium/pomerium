...
// IMPORTANT: these tests are written against a future version of this code, which has improved
// logic in several areas. These tests will check for specific sequences of auth method attempts to
// exercise different code paths for various policy configurations. The current version does not
// always generate the same sequences. In particular, it will generally accept the first public key
// it is given, then move on to keyboard-interactive, and only evaluate the public key against the
// route's policy after keyboard-interactive is done. The future version can evaluate public keys
// and negotiate retries before moving on to keyboard-interactive, as well as handle multiple
// keyboard-interactive prompts, and other things. Whether the request ultimately succeeds or fails
// should not change between versions, so the sequence assertions are disabled for now, but the
// error checks remain. There are a couple scenarios tested here that the current version cannot
...
func seqPublicKeyAcceptedAfter1RetryThenKbdInit(t *testing.T, keys [2]gossh.Signer, kbdInt gossh.AuthMethod) []func(*gossh.ClientAuthContext) gossh.AuthMethod {
	if !future {
		// if the test case calls this at all, the scenario is not supported
		t.Skip("not implemented in this version")
	}
...
const (
	sshErrMsgPublicKeyAuthFailedNoMethodsRemain = "ssh: unable to authenticate, attempted methods [none publickey], no supported methods remain"
	sshErrMsgDisconnectPermissionDenied         = "ssh: handshake failed: ssh: disconnect, reason 2: \"Permission Denied\""
)

var sshErrMsgPublicKeyAuthFailed = func() string {
	if !future {
		return sshErrMsgDisconnectPermissionDenied
	}
	return sshErrMsgPublicKeyAuthFailedNoMethodsRemain
}()
...
`, func(api RouteTestAPI) {
		...
		s.Run("public key accepted, but unauthorized due to missing session criteria", func() {
			cc := s.newClientConfig("username", api.RouteName(), api.RouteUserEmail(2))
			if !future {
				// Enable only the publickey handler here. In the current version this is equivalent to
				// "allow any authenticated user" so if the kbdint handler is enabled, it will just be used
				// as normal and the client will be allowed in, and the sequence asserts are disabled so
				// that check doesn't happen. In the future this is treated as an invalid configuration and
				// keyboard-interactive is never reached (but in that case we want to keep the handler
				// enabled to test that the sequence doesn't reach the point where it would be used)
				cc.Auth = []gossh.AuthMethod{cc.Auth[0]}
			}
			...
			_, err := s.upstream.Dial(cc)
			if !future {
				s.ErrorContains(err, "attempted methods [none], no supported methods remain")
			} else {
				s.ErrorContains(err, "Permission Denied")
			}
		})
...
`,
	...
	// Try to log into other routes which are not authorized for this session
	{
		...
		_, err := s.upstream.Dial(cc)
		if !future {
			s.Require().ErrorContains(err, sshErrMsgPublicKeyAuthFailedNoMethodsRemain)
		} else {
...
`,
	...
	_, err := s.upstream.Dial(cc)
	if !future {
		s.ErrorContains(err, sshErrMsgDisconnectPermissionDenied)
	} else {
...
`,
	...
	s.Run("authorized by pomerium, but username not allowed by upstream", func() {
		...
		if !future {
			s.ErrorContains(err, sshErrMsgPublicKeyAuthFailedNoMethodsRemain)
		} else {
			s.ErrorContains(err, sshErrMsgPublicKeyAuthFailed)
		}
	})
