...
// Access request arbitration (two-person approval) callbacks
type PolicyIndexArbitrationSubscriber interface {
	UpdateArbitrationAuthorizedRoutes(routes []common.RouteInfo)
}

type PolicyIndexTopic int

const (
	TopicUpstreamTunnel PolicyIndexTopic = iota
	TopicAccessRequestArbitration
)

type PolicyIndexSubscriber interface {
	PolicyIndexTunnelSubscriber
	PolicyIndexArbitrationSubscriber
}
...
type PolicyIndexSubscriberSet struct {
	...
	Arbitration PolicyIndexArbitrationSubscriber
}

// UpdateArbitrationAuthorizedRoutes implements [PolicyIndexSubscriber].
func (p *PolicyIndexSubscriberSet) UpdateArbitrationAuthorizedRoutes(routes []common.RouteInfo) {
	if p.Arbitration != nil {
		p.Arbitration.UpdateArbitrationAuthorizedRoutes(routes)
	}
}
