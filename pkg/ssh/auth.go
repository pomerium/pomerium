...
type SSHEvaluator interface {
	...
	// Evaluates whether a user can approve or deny access requests to this route
	EvaluateAccessRequestArbitration(ctx context.Context, req AuthRequest, route *config.Policy) (*evaluator.Result, error)
}
...
// AccessRequestManager implements [AuthInterface].
func (a *Auth) AccessRequestManager() api.AccessRequestManagerInterface {
	return a.accessRequestMgr
}
...
func (a *Auth) handleKeyboardInteractiveMethodRequest(
	...
	if !authInfoHasSession(authInfo) {
		...
		if authInfo.GetAccessRequestState() == extensions_ssh.AccessRequestState_Pending {
			...
			err := a.handleTwoPersonApproval(ctx, policy, streamInfo, authInfo, user, querier)
			if err != nil {
				return AuthMethodResponse{}, err
			}
...
func processSessionEvaluateResult(
	...
	if res.Allow.Value {
		...
		// Check if two-person auth is required. Note that it is only valid as a
		// deny reason, so Allow has to be true. Additionally, it must be the only
		// remaining deny reason. The presence of any other deny reason constitutes
		// auth failure.
		if len(res.Deny.Reasons) == 1 && res.Deny.Reasons.Has(criteria.ReasonSSHAccessRequestRequired) {
...
func (a *Auth) handleTwoPersonApproval(
	...
	if policy.TwoPersonApproval.Timeout != nil {
		timeout = *policy.TwoPersonApproval.Timeout
	}
	ctx, ca := context.WithTimeout(ctx, timeout)
	defer ca()

	_, err := querier.Prompt(ctx, &extensions_ssh.KeyboardInteractiveInfoPrompts{
		Name:        fmt.Sprintf("\nWaiting for approval (timeout: %s)", timeout),
		Instruction: "",
		Prompts:     nil,
	})
	if err != nil {
	...
	now := timestamppb.Now()
	return a.accessRequestMgr.DoRequest(ctx, &session.StreamAccessRequest{
		...
		CreatedAt: now,
		ExpiresAt: timestamppb.New(now.AsTime().Add(timeout)),
	})
}
