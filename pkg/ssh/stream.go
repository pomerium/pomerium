...
type AuthInterface interface {
	HandlePublicKeyMethodRequest(ctx context.Context, info DownstreamInfo, req *extensions_ssh.PublicKeyMethodRequest) (PublicKeyAuthMethodResponse, error)
	HandleKeyboardInteractiveMethodRequest(ctx context.Context, info DownstreamInfo, req *extensions_ssh.KeyboardInteractiveMethodRequest, querier KeyboardInteractiveQuerier) (KeyboardInteractiveAuthMethodResponse, error)
...
func (sh *StreamHandler) handleAuthRequest(ctx context.Context, req *extensions_ssh.AuthenticationRequest) error {
	...
	case MethodPublicKey:
		...
		response, err := sh.auth.HandlePublicKeyMethodRequest(ctx, sh.state.DownstreamInfo, pubkeyReq)
		if err != nil {
			return err
		}
	...
	case MethodKeyboardInteractive:
		...
		response, err := sh.auth.HandleKeyboardInteractiveMethodRequest(ctx, sh.state.DownstreamInfo, kbiReq, sh)
		if err != nil {
			return err
		}
		partial = response.Allow != nil
