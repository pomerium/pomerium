...
type RecordSetBundle[T IRecord[T]] map[string]RecordSet[T]

// RecordSet is an index of databroker records by their id.
type RecordSet[T IRecord[T]] map[string]T

// IRecord is an abstract record
type IRecord[T any] interface {
	GetID() string
	GetType() string
	Equal(other T) bool
}
