...
func NewBucketFromConfig(cfg *blob.StorageConfig) (objstore.Bucket, error) {
	if cfg == nil {
		return nil, fmt.Errorf("blob storage config is not set")
	}

	provider := objstore.ObjProvider(strings.ToUpper(cfg.Provider))
	bucket := cfg.Bucket

	var providerConfig any
	switch provider {
	case objstore.S3:
		providerConfig = defaultS3Config(bucket, cfg.S3)
	case objstore.GCS:
