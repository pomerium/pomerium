...
# tracer), so they must run one at a time in their own process. They are gated
# behind the `e2e` build tag AND a per-test RUN_<TestName> env var. Each has its
# own sub-target so they never share a process; test-e2e chains them.
.PHONY: test-e2e
...
test-e2e-k3s: get-envoy ## Runs the k3s external-JWT e2e test (requires Docker).
	@echo "==> $@"
	RUN_TestExternalJWTBearer_K3s=1 $(GO) test -tags e2e -timeout=15m -run '^TestExternalJWTBearer_K3s$$' ./authorize/...
