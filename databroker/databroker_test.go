...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		{"good", config.Options{SharedKey: cryptutil.NewBase64Key(), DataBroker: config.DataBrokerOptions{URLString: "http://example"}}, false},
		{"bad shared secret", config.Options{SharedKey: string([]byte(cryptutil.NewBase64Key())[:31]), DataBroker: config.DataBrokerOptions{URLString: "http://example"}}, true},
	}
