...
func New(ctx context.Context, cfg *config.Config, opts ...Option) (*Authorize, error) {
	...
	state, err := newAuthorizeStateFromConfig(ctx, nil, tracerProvider, cfg, a.store, a.secretsResolver, &a.outboundGrpcConn, a)
	if err != nil {
		return nil, err
	}
	a.state.Store(state)
...
func (a *Authorize) OnConfigChange(ctx context.Context, cfg *config.Config) {
	...
	// fail-closed 503s).
	a.applySecrets(ctx, cfg)
	if newState, err := newAuthorizeStateFromConfig(ctx, currentState, a.tracerProvider, cfg, a.store, a.secretsResolver, &a.outboundGrpcConn, a); err != nil {
