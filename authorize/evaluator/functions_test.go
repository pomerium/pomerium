...
func Test_isValidClientCertificate(t *testing.T) {
	t.Run("no ca", func(t *testing.T) {
		valid, err := isValidClientCertificate("", "", ClientCertificateInfo{Leaf: "WHATEVER!"}, 0)
		assert.NoError(t, err, "should not return an error")
		assert.True(t, valid, "should return true")
	})
	t.Run("no cert", func(t *testing.T) {
		valid, err := isValidClientCertificate(testCA, "", ClientCertificateInfo{}, 0)
		assert.NoError(t, err, "should not return an error")
	...
	t.Run("revoked cert", func(t *testing.T) {
		...
		valid, err := isValidClientCertificate(testCA, "", revokedCertInfo, 0)
		assert.NoError(t, err, "should not return an error")
		assert.True(t, valid, "should return true")

		valid, err = isValidClientCertificate(testCA, testCRL, revokedCertInfo, 0)
		assert.NoError(t, err, "should not return an error")
		assert.False(t, valid, "should return false")
	...
	t.Run("max verify depth", func(t *testing.T) {
		valid, err := isValidClientCertificate(testCA, "", ClientCertificateInfo{
			Presented:     true,
			Validated:     true,
			Leaf:          testValidIntermediateCert,
