...
func Test_isValidClientCertificate(t *testing.T) {
	...
	t.Run("any SAN", func(t *testing.T) {
		matchAnySAN := ClientCertConstraints{SANMatchers: SANMatchers{
			configpb.SANMatcher_DNS:        regexp.MustCompile("^.*$"),
			configpb.SANMatcher_EMAIL:      regexp.MustCompile("^.*$"),
			configpb.SANMatcher_IP_ADDRESS: regexp.MustCompile("^.*$"),
			configpb.SANMatcher_URI:        regexp.MustCompile("^.*$"),
		}}
	...
	t.Run("DNS SAN", func(t *testing.T) {
		valid, err := isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_DNS: regexp.MustCompile(`^a\..*\.example\.com$`),
		}})
		assert.NoError(t, err, "should not return an error")
		assert.True(t, valid, "should return true")
		...
		valid, err = isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_EMAIL: regexp.MustCompile(`^a\..*\.example\.com$`), // mismatched type
		}})
		assert.NoError(t, err, "should not return an error")
		assert.False(t, valid, "should return false")
	})
	t.Run("email SAN", func(t *testing.T) {
		valid, err := isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_EMAIL: regexp.MustCompile(`^.*@example\.com$`),
		}})
		assert.NoError(t, err, "should not return an error")
		assert.True(t, valid, "should return true")
		...
		valid, err = isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_IP_ADDRESS: regexp.MustCompile(`^.*@example\.com$`), // mismatched type
		}})
		assert.NoError(t, err, "should not return an error")
		assert.False(t, valid, "should return false")
	})
	t.Run("IP address SAN", func(t *testing.T) {
		valid, err := isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_IP_ADDRESS: regexp.MustCompile(`^192\.168\.10\..*$`),
		}})
		assert.NoError(t, err, "should not return an error")
		assert.True(t, valid, "should return true")
		...
		valid, err = isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_URI: regexp.MustCompile(`^192\.168\.10\..*$`), // mismatched type
		}})
		assert.NoError(t, err, "should not return an error")
		assert.False(t, valid, "should return false")
	})
	t.Run("URI SAN", func(t *testing.T) {
		valid, err := isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_URI: regexp.MustCompile(`^spiffe://example\.com/.*$`),
		}})
		assert.NoError(t, err, "should not return an error")
		assert.True(t, valid, "should return true")
		...
		valid, err = isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_DNS: regexp.MustCompile(`^spiffe://example\.com/.*$`), // mismatched type
		}})
		assert.NoError(t, err, "should not return an error")
		assert.False(t, valid, "should return false")
	})
	t.Run("UserPrincipalName SAN", func(t *testing.T) {
		valid, err := isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_USER_PRINCIPAL_NAME: regexp.MustCompile(`^test_device$`),
		}})
		assert.NoError(t, err, "should not return an error")
		assert.True(t, valid, "should return true")
		...
		valid, err = isValidClientCertificate(testCA, "", ClientCertificateInfo{
			...
			configpb.SANMatcher_DNS: regexp.MustCompile(`^test-device$`), // mismatched type
		}})
		assert.NoError(t, err, "should not return an error")
		assert.False(t, valid, "should return false")
	})
...
func TestClientCertConstraintsFromConfig(t *testing.T) {
	...
	t.Run("one SAN match", func(t *testing.T) {
		...
		assert.Equal(t, &ClientCertConstraints{
			...
			SANMatchers: SANMatchers{
				configpb.SANMatcher_DNS: regexp.MustCompile(`^(.*\.corp\.example\.com)$`),
			},
		}, c)
	})
	t.Run("multiple SAN matches", func(t *testing.T) {
		s := config.DownstreamMTLSSettings{
			MatchSubjectAltNames: []config.SANMatcher{
				{Type: nullable.From(configpb.SANMatcher_DNS), Pattern: `.*\.foo\.example\.com`},
				{Type: nullable.From(configpb.SANMatcher_DNS), Pattern: `.*\.bar\.example\.com`},
			},
		}
		c, err := ClientCertConstraintsFromConfig(&s)
		assert.NoError(t, err)
		assert.Equal(t, &ClientCertConstraints{
			MaxVerifyDepth: 1,
			SANMatchers: SANMatchers{
				configpb.SANMatcher_DNS: regexp.MustCompile(`^(.*\.foo\.example\.com)|(.*\.bar\.example\.com)$`),
			},
		}, c)
	})
	t.Run("multiple SAN matches", func(t *testing.T) {
		...
		assert.Equal(t, &ClientCertConstraints{
			MaxVerifyDepth: 1,
			SANMatchers: SANMatchers{
				configpb.SANMatcher_DNS:   regexp.MustCompile(`^(.*\.foo\.example\.com)$`),
				configpb.SANMatcher_EMAIL: regexp.MustCompile(`^(.*@example\.com)$`),
			},
		}, c)
	})
